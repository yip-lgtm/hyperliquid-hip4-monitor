...
async function yahooFinanceFetch(urlBase, params = {}, moduleOpts = {}, func = "json", needsCrumb = false) {
    ...
    const queueOverride = moduleOpts.queue
        ?._queue;
    const queue = queueOverride instanceof queue_js_1.default ? queueOverride : _queue;
    // const queue = _queue;
    ...
    if (needsCrumb) {
        ...
        const crumb = await (0, getCrumb_js_1.default)(this._opts.cookieJar, queuedFetch, fetchOptionsBase, this._opts.logger, this._notices);
        if (crumb)
            params.crumb = crumb;
    }
